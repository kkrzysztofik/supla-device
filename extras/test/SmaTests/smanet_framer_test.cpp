...
TEST(SmaNetFramerTest, EncapsulateAndDecode) {
  const uint8_t smadata[] = {
      0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x0b, 0x01, 0x08, 0x00};
  SmaNetFramer framer;
  const auto wire =
...
TEST(SmaNetFramerTest, EscapesHdlcControlBytesInPayload) {
  const uint8_t smadata[] = {Supla::Linux::Sma::kHdlcSync,
                             Supla::Linux::Sma::kHdlcEsc,
                             0x00,
                             0x7d,
                             0x7e};
  SmaNetFramer framer;
  ...
  FAIL() << "No escaped SMANet frame decoded";
}

TEST(SmaNetFramerTest, RejectsCorruptedChecksum) {
  const uint8_t smadata[] = {0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x0b};
  SmaNetFramer framer;
  auto wire = framer.encapsulate(kProtPppSmadata1, smadata, sizeof(smadata));
  ...
  for (uint8_t byte : wire) {
    rx.feed(byte);
    ASSERT_FALSE(rx.takeFrame().has_value());
  }
}
...
TEST(SmaChannelCodecTest, ParseFloatSpotChannel) {
  uint8_t data[] = {
      0x01,
      0x08,  // ctype CH_SPOT|CH_IN|CH_ANALOG
      0x00,  // cindex
      0x01,
      0x00,  // dataset count
      0x00,
  ...
  double value = 0.0;
  ASSERT_TRUE(
      SmaChannelCodec::parseGetDataValue(data, sizeof(data), channel, &value));
