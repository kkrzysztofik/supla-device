...
std::optional<std::vector<SmaChannelInfo>> SmaCinfoParser::parse(
  ...
  while (pos < len) {
    ...
    info.descriptor.ctype = SmaChannelCodec::le16ToHost(&data[pos + 1]);
    info.descriptor.ntype =
        static_cast<uint16_t>(SmaChannelCodec::le16ToHost(&data[pos + 3]));
    info.name =
        trimSmaChannelName(reinterpret_cast<const char*>(&data[pos + 7]), 16);
    pos += 23;
    ...
    if (info.descriptor.ctype & kChAnalog) {
      ...
      info.descriptor.gain = SmaChannelCodec::le32fToHost(&data[pos + 8]);
      info.descriptor.offset = SmaChannelCodec::le32fToHost(&data[pos + 12]);
      pos += 16;
      ...
      const uint16_t stateCount = SmaChannelCodec::le16ToHost(&data[pos]);
      pos += 2;
...
const SmaChannelInfo* SmaCinfoParser::findByName(
    const std::vector<SmaChannelInfo>& channels, const std::string& name) {
